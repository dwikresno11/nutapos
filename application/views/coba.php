    ...
    <div class="col-sm-8" style="margin: auto;">
      <div class="row">
        <div class="col-xs-6" style="margin-left: auto;margin-right: auto;">
          ...
          <div class="col-xs-6" style="margin-left: auto;margin-right: auto;">
            <?php include "table_schedule.php" ?>
           <!--  <div class="tab-content" style="height: 490px">
             <div class='tabs'>
              <input type='radio' id='r1' name='t' checked>
              <label id="label-tab" for='r1'>Lapangan Hijau</label>
              <div class='content'>
               </div>
             <input type='radio' id='r2' name='t'>
             <label id="label-tab" for='r2'>Lapangan Biru</label>
             <div class='content'> 
              <?php 
...
<footer class="footer" style="padding: 0px;padding-top: 10%">
  <div class="container">
    <div class="row mb-5">
      ...
      <div class="col-md-6 col-lg-4">
        ...
        <div class="block-21 d-flex mb-4">
          <figure class="mr-3">
            <img src="<?php echo base_url()?>assets/images/img_3.jpg" alt="" class="img-fluid">
          </figure>
          <div class="text">
            <h3 class="heading"><a href="#">Our Luxury Hotel &amp; Restaurants</a></h3>
            <div class="meta">
...
<script type="text/javascript">
  $(document).ready( function () {
    ...
    $('#jadwal').on( 'click', 'tr>td:nth-child(3)', function () {
      var id = this.id;
      var tanggal = "<?php echo $_POST['tanggal']?>";
      document.getElementById('id_jadwal').value=id;
      var session_login='<?php echo $this->session->has_userdata('login'); ?>';
      if(session_login==""){
        $('#myModal1').modal('show');
      }else{
        var exp_date='<?php if($this->session->has_userdata('login')){ echo $this->session->userdata('login')[0]->exp_date; }?>';
        console.log(exp_date);
        var id_jadwal = document.getElementById("id_jadwal").value;
        var email = '<?php if($this->session->has_userdata('login')){ echo $this->session->userdata('login')[0]->email; }?>';
        var no_tlp = '<?php if($this->session->has_userdata('login')){ echo $this->session->userdata('login')[0]->no_tlp; } ?>';
        var nama = '<?php if($this->session->has_userdata('login')){ echo $this->session->userdata('login')[0]->nama; } ?>';
        var tanggal = '<?php echo $_POST['tanggal']?>';
        if(id_jadwal!=""){
          var r = confirm("Yakin ingin melakukan pemesan untuk tanggal "+tanggal+" pukul "+id_jadwal+".00 ?");
          if (r == true) {
            console.log(exp_date);
            if(exp_date==""){
              $.ajax({
                ...
                data:{date:tanggal,time:id_jadwal},
                dataType: "html",
